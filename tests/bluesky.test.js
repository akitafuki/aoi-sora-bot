...
const bluesky_1 = require("../src/bluesky");
...
jest.mock('@atproto/api', () => {
    ...
    global.mockBskyLogin = login;
    global.mockBskyGetAuthorFeed = getAuthorFeed;
    return {
...
const mockLogin = global.mockBskyLogin;
const mockGetAuthorFeed = global.mockBskyGetAuthorFeed;
describe('bluesky.ts Unit Tests', () => {
    ...
    describe('authenticateBluesky', () => {
        it('should call login on the agent', async () => {
            await (0, bluesky_1.authenticateBluesky)();
    ...
    describe('getLatestPosts', () => {
        it('should fetch posts and return them in reverse (chronological) order', async () => {
            ...
            const posts = await (0, bluesky_1.getLatestPosts)(null, { ...defaultSettings, ignoreReplies: false, ignoreReposts: false });
        ...
        it('should stop processing once sinceUri is reached', async () => {
            ...
            mockGetAuthorFeed.mockResolvedValue(mockFeed);
            const posts = await (0, bluesky_1.getLatestPosts)('at://already-processed', {
                ...defaultSettings,
                ignoreReplies: false,
                ignoreReposts: false,
            });
        ...
        it('should filter out reposts if ignoreReposts is true', async () => {
            ...
            mockGetAuthorFeed.mockResolvedValue(mockFeed);
            const posts = await (0, bluesky_1.getLatestPosts)(null, { ...defaultSettings, ignoreReposts: true, ignoreReplies: false });
            expect(posts).toHaveLength(1);
        ...
        it('should filter out replies if ignoreReplies is true', async () => {
            ...
            mockGetAuthorFeed.mockResolvedValue(mockFeed);
            const posts = await (0, bluesky_1.getLatestPosts)(null, { ...defaultSettings, ignoreReplies: true, ignoreReposts: false });
            expect(posts).toHaveLength(1);
            expect(posts[0].post.uri).toBe('at://post-regular');
        });
        it('should filter out posts containing ignoreKeywords', async () => {
            ...
            mockGetAuthorFeed.mockResolvedValue(mockFeed);
            const posts = await (0, bluesky_1.getLatestPosts)(null, {
                ...defaultSettings,
                ignoreReplies: false,
                ignoreReposts: false,
